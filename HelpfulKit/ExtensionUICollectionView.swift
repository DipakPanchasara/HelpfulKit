...
public extension UICollectionView {
    ...
    func visibleIndex() -> IndexPath? {
        ...
        let visiblePoint = CGPointMake(CGRectGetMidX(visibleRect),
    ...
    func register(_ nibAndReuse: String) {
        self.register(UINib(nibName: nibAndReuse, bundle: Bundle.main), forCellWithReuseIdentifier: nibAndReuse)
    }
    func register(multiple: String...) {
        multiple.forEach { register($0) }
    }
    func registerforHeaderView(_ nibAndReuse: String) {
        self.register(UINib(nibName: nibAndReuse, bundle: .main),
                      forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader,
                      withReuseIdentifier: nibAndReuse)
    }
